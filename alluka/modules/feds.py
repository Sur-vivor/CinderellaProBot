...
def del_fed(update, context):
	...
	args = context.args
	if chat.type != "private":
		send_message(update.effective_message, "Hapus federasi Anda di PM saya, bukan dalam grup.")
		return
	if args:
		is_fed_id = args[0]
		getinfo = sql.get_fed_info(is_fed_id)
		if getinfo == False:
			send_message(update.effective_message, "Federasi ini tidak di temukan!")
			return
		...
		send_message(update.effective_message, "Apa yang harus saya hapus?")
		return
	...
	send_message(update.effective_message, "Anda yakin ingin menghapus federasi Anda? Tindakan ini tidak bisa dibatalkan, Anda akan kehilangan seluruh daftar larangan Anda, dan '{}' akan hilang secara permanen.").format(getinfo['fname']),
...
def fed_chat(update, context):
	...
	if not is_user_admin(update.effective_chat, user_id):
		send_message(update.effective_message, "Anda harus menjadi admin untuk menjalankan perintah ini")
		return
	...
	text = (update.effective_message, "Obrolan ini adalah bagian dari federasi berikut:")
...
def join_fed(update, context):
	...
	if chat.type == 'private':
		send_message(update.effective_message, "Perintah ini di khususkan untuk grup, bukan pada PM!")
		return
		...
		for admin in administrators:
			...
			if status == "creator":
				if str(admin.user.id) == str(user.id):
					...
					send_message(update.effective_message, "Hanya pembuat grup yang dapat melakukannya!")
					return
	if fed_id:
		send_message(update.effective_message, "Anda tidak bisa bergabung dua federasi dalam satu obrolan")
		return

	if len(args) >= 1:
		getfed = sql.search_fed_by_id(args[0])
		if getfed == False:
			send_message(update.effective_message, "Silakan masukkan id federasi yang valid.")
			return

		x = sql.chat_join_fed(args[0], chat.title, chat.id)
		if not x:
			send_message(update.effective_message, "Gagal bergabung dengan federasi! Tolong hubungi pembuat saya jika masalah ini masih berlanjut.")
			return

		get_fedlog = sql.get_fed_log(args[0])
		if get_fedlog:
			if eval(get_fedlog):
				context.bot.send_message(get_fedlog, (update.effective_message, "Obrolan *{}* telah bergabung ke federasi *{}*").format(chat.title, getfed['fname']), parse_mode="markdown")

		send_message(update.effective_message, "Obrolan ini telah bergabung dengan federasi {}!").format(getfed['fname'])
...
def leave_fed(update, context):
	...
	if getuser in 'creator' or user.id in SUDO_USERS:
		if sql.chat_leave_fed(chat.id) == True:
			...
			if get_fedlog:
				if eval(get_fedlog):
					context.bot.send_message(get_fedlog, update.effective_message, "Obrolan *{}* telah keluar ke federasi *{}*").format(chat.title, fed_info['fname']), parse_mode="markdown"
			send_message(update.effective_message, "Obrolan ini telah keluar dari federasi {}!").format(fed_info['fname'])
		else:
			send_message(update.effective_message, "Mengapa Anda meninggalkan federasi ketika Anda belum bergabung?!")
	else:
		send_message(update.effective_message, "Hanya pembuat grup yang dapat melakukannya!")
...
def user_join_fed(update, context):
	...
	if is_user_fed_owner(fed_id, user.id):
		...
		if user_id and user_id != "error":
			...
			send_message(update.effective_message, "Saya tidak dapat mengekstrak pengguna dari ini.")
			return
		else:
		...
		if user_id == get_owner:
			send_message(update.effective_message, "Mengapa Anda mencoba mempromosikan pemilik federasi!?")
			return
		if getuser:
			send_message(update.effective_message, "Saya tidak dapat mempromosikan pengguna yang sudah menjadi admin federasi! Tapi saya bisa menurunkannya.")
			return
		if user_id == context.bot.id:
			send_message(update.effective_message, "Saya sudah menjadi admin federasi dan yang mengelolanya!")
			return
		res = sql.user_join_fed(fed_id, user_id)
		if res:
			send_message(update.effective_message, "💖 Berhasil dinaikan jabatannya!")
		else:
			send_message(update.effective_message, "Gagal dipromosikan!")
	else:
		send_message(update.effective_message, "Hanya pemilik federasi yang dapat melakukan ini!")
...
def user_demote_fed(update, context):
	...
	if is_user_fed_owner(fed_id, user.id):
		...
		if user_id == context.bot.id:
			send_message(update.effective_message, "Apa yang sedang Anda coba lakukan? Menurunkan saya dari federasi Anda?")
			return

		if sql.search_user_in_fed(fed_id, user_id) == False:
			send_message(update.effective_message, "Saya tidak dapat mendemosikan pengguna yang bukan merupakan admin federasi! Jika Anda ingin membuatnya menangis, promosikan dia terlebih dahulu!")
			return

		res = sql.user_demote_fed(fed_id, user_id)
		if res == True:
			send_message(update.effective_message, "💔 Dia telah di keluarkan dari federasi anda!")
		else:
			send_message(update.effective_message, "Saya tidak bisa mengusirnya, Saya tidak berdaya!")
	else:
		send_message(update.effective_message, "Hanya pemilik federasi yang dapat melakukan ini!")
		return
...
def fed_info(update, context):
	...
	text = (update.effective_message, "<b>ℹ️ Info federasi:</b>")
...
def fed_admin(update, context):
	...
	if is_user_fed_admin(fed_id, user.id) == False:
		send_message(update.effective_message, "Hanya admin federasi yang dapat melakukan ini!")
		return
	...
	text = (update.effective_message, "<b>Admin Federasi {}:</b>\n\n").format(info['fname'])
	text += "👑 Owner:\n"
	owner = context.bot.get_chat(info['owner'])
	try:
		owner_name = owner.first_name + " " + owner.last_name
...
def fed_ban(update, context):
	...
	if not user_id:
		send_message(update.effective_message, "Anda sepertinya tidak merujuk ke pengguna.")
		return

	if user_id == context.bot.id:
		send_message(update.effective_message, "Apa yang lebih lucu dari menendang creator grup? Fban diri saya sendiri.")
		return

	if is_user_fed_owner(fed_id, user_id) == True:
		send_message(update.effective_message, "Mengapa Anda mencoba fban pemilik federasi?")
		return

	if is_user_fed_admin(fed_id, user_id) == True:
		send_message(update.effective_message, "Dia adalah admin dari federasi, saya tidak bisa fban dia.")
		return

	if user_id == OWNER_ID:
		send_message(update.effective_message, "Aku tidak ingin memblokir tuanku, itu ide yang sangat bodoh!")
		return

	if int(user_id) in SUDO_USERS:
		send_message(update.effective_message, "Saya tidak akan fban pengguna sudo!")
		return

	if int(user_id) in WHITELIST_USERS:
		send_message(update.effective_message, "Orang ini masuk daftar putih, jadi tidak bisa di fban!")
		return
	...
	if fban:
		fed_name = info['fname']
		starting = (update.effective_message, "Alasan fban di ganti untuk {} pada Federasi <b>{}</b>.").format(user_target, fed_name)
		send_message(update.effective_message, starting, parse_mode=ParseMode.HTML)

		if reason == "":
			reason = (update.effective_message, "Tidak ada alasan.")

		temp = sql.un_fban_user(fed_id, fban_user_id)
		if not temp:
			send_message(update.effective_message, "Gagal mengupdate alasan fedban!")
			return
		x = sql.fban_user(fed_id, fban_user_id, fban_user_name, fban_user_lname, fban_user_uname, reason, int(time.time()))
		if not x:
			send_message(update.effective_message, "Gagal melarangan federasi! Jika masalah ini terus terjadi, hubungi pembuat saya.")
			return

		fed_chats = sql.all_fed_chats(fed_id)
		# Will send to current chat
		context.bot.send_message(chat.id, (update.effective_message, "<b>Alasan FedBan Diperbarui</b>" \
								  "\n<b>Federasi:</b> {}" \
								  "\n<b>Federasi Admin:</b> {}" \
								  "\n<b>Pengguna:</b> {}" \
		...
		if getfednotif:
			context.bot.send_message(info['owner'], (update.effective_message, "<b>Alasan FedBan Diperbarui</b>" \
											"\n<b>Federasi:</b> {}" \
											"\n<b>Federasi Admin:</b> {}" \
											"\n<b>Pengguna:</b> {}" \
											"\n<b>Pengguna ID:</b> <code>{}</code>" \
		...
		if get_fedlog:
			if int(get_fedlog) != int(chat.id):
				context.bot.send_message(get_fedlog, (update.effective_message, "<b>Alasan FedBan Diperbarui</b>" \
											"\n<b>Federasi:</b> {}" \
											"\n<b>Federasi Admin:</b> {}" \
											"\n<b>Pengguna:</b> {}" \
											"\n<b>Pengguna ID:</b> <code>{}</code>" \
			...
			for fedsid in subscriber:
				...
				for fedschat in all_fedschat:
					try:
						...
						pass
		send_message(update.effective_message, "Alasan fedban telah di perbarui.")
		return

	fed_name = info['fname']

	starting = (update.effective_message, "Memulai larangan federasi untuk {} pada Federasi <b>{}</b>.").format(user_target, fed_name)
	send_message(update.effective_message, starting, parse_mode=ParseMode.HTML)
	...
	fed_chats = sql.all_fed_chats(fed_id)
	# Will send to current chat
	context.bot.send_message(chat.id, (update.effective_message, "<b>FedBan baru</b>" \
							  "\n<b>Federasi:</b> {}" \
							  "\n<b>Federasi Admin:</b> {}" \
							  "\n<b>Pengguna:</b> {}" \
							  "\n<b>Pengguna ID:</b> <code>{}</code>" \
	...
	# Send message to owner if fednotif is enabled
	if getfednotif:
		context.bot.send_message(info['owner'], (update.effective_message, "<b>FedBan baru</b>" \
										"\n<b>Federasi:</b> {}" \
										"\n<b>Federasi Admin:</b> {}" \
										"\n<b>Pengguna:</b> {}" \
										"\n<b>Pengguna ID:</b> <code>{}</code>" \
	...
	if get_fedlog:
		if int(get_fedlog) != int(chat.id):
			context.bot.send_message(get_fedlog, (update.effective_message, "<b>FedBan baru</b>" \
										"\n<b>Federasi:</b> {}" \
										"\n<b>Federasi Admin:</b> {}" \
										"\n<b>Pengguna:</b> {}" \
										"\n<b>Pengguna ID:</b> <code>{}</code>" \
		...
		for fedsid in subscriber:
			...
			for fedschat in all_fedschat:
				try:
					...
					pass
	send_message(update.effective_message, "Orang ini telah di fbanned.")
...
def unfban(update, context):
	...
	if fban == False:
		send_message(update.effective_message, "Pengguna ini tidak di fbanned!")
		return

	banner = update.effective_user  # type: Optional[User]

	send_message(update.effective_message, "Saya akan memberi {} kesempatan kedua dalam federasi ini.").format(user_target), parse_mode="HTML"

	chat_list = sql.all_fed_chats(fed_id)
	# Will send to current chat
	context.bot.send_message(chat.id, (update.effective_message, "<b>Un-FedBan</b>" \
						 "\n<b>Federasi:</b> {}" \
						 "\n<b>Federasi Admin:</b> {}" \
						 "\n<b>Pengguna:</b> {}" \
	...
	# Send message to owner if fednotif is enabled
	if getfednotif:
		context.bot.send_message(info['owner'], (update.effective_message, "<b>Un-FedBan</b>" \
						 "\n<b>Federasi:</b> {}" \
						 "\n<b>Federasi Admin:</b> {}" \
						 "\n<b>Pengguna:</b> {}" \
						 "\n<b>Pengguna ID:</b> <code>{}</code>").format(info['fname'], mention_html(user.id, user.first_name), user_target, fban_user_id), parse_mode="HTML")
	...
	if get_fedlog:
		if int(get_fedlog) != int(chat.id):
			context.bot.send_message(get_fedlog, (update.effective_message, "<b>Un-FedBan</b>" \
						 "\n<b>Federasi:</b> {}" \
						 "\n<b>Federasi Admin:</b> {}" \
						 "\n<b>Pengguna:</b> {}" \
						 "\n<b>Pengguna ID:</b> <code>{}</code>").format(info['fname'], mention_html(user.id, user.first_name), user_target, fban_user_id), parse_mode="HTML")
	...
	try:
		...
		if not x:
			send_message(update.effective_message, "Gagal fban, Pengguna ini mungkin sudah di un-fedbanned!")
			return
	except:
		pass
	...
	send_message(update.effective_message, "Orang ini telah di un-fbanned.")
	# Also do not spamming all fed admins
	"""
	FEDADMIN = sql.all_fed_users(fed_id)
	for x in FEDADMIN:
...
def set_frules(update, context):
	...
	if not fed_id:
		send_message(update.effective_message, "Obrolan ini tidak ada di federasi apa pun!")
		return
	...
	if len(args) >= 1:
		...
		if get_fedlog:
			if eval(get_fedlog):
				context.bot.send_message(get_fedlog, (update.effective_message, "*{}* telah mengganti aturan federasi *{}*").format(user.first_name, getfed['fname']), parse_mode="markdown")
		send_message(update.effective_message, "Aturan telah di ganti menjadi:\n{}!").format(rules)
	else:
		send_message(update.effective_message, "Silakan tulis aturan untuk mengaturnya!")
...
def get_frules(update, context):
	...
	text = (update.effective_message, "*Peraturan di fed ini:*\n")
	text += rules
...
def fed_broadcast(update, context):
	...
	if args:
		...
		for chat in chat_list:
			title = (chat, "*Siaran baru dari Federasi {}*\n").format(fedinfo['fname'])
			try:
		...
		send_text = (update.effective_message, "Siaran Federasi selesai.")
		if failed >= 1:
			send_text += (update.effective_message, "{} grup gagal menerima pesan, mungkin karena meninggalkan federasi.").format(failed)
...
def fed_ban_list(update, context):
	...
	getfban = sql.get_all_fban_users(fed_id)
	if len(getfban) == 0:
		send_message(update.effective_message, "Tidak ada pengguna yang di fban di federasi {}").format(info['fname']), parse_mode=ParseMode.HTML
		return
		...
		if args[0] == 'json':
			...
			with BytesIO(str.encode(backups)) as output:
				...
				update.effective_message.reply_document(document=output, filename="alluka_fbanned_users.json",
													caption=(update.effective_message, "Total {} pengguna terkena blokir federasi {}.").format(len(getfban), info['fname']))
			return
		elif args[0] == 'csv':
			jam = time.time()
			new_jam = jam + 1800
	...
	for users in getfban:
		...
		if getuserinfo == False:
			text = (update.effective_message, "Tidak ada pengguna yang di fban di federasi {}").format(info['fname'])
			break
	...
	try:
		...
		with BytesIO(str.encode(cleantext)) as output:
			...
			update.effective_message.reply_document(document=output, filename="fbanlist.txt",
													caption=(update.effective_message, "Berikut adalah daftar pengguna yang saat ini difban pada federasi {}.").format(info['fname']))
...
def fed_notif(update, context):
	...
	if args:
		if args[0] in ("yes", "on", "ya"):
			...
			send_message(update.effective_message, "Pelaporan federasi hidup! Setiap ada pengguna yang di fban/unfban anda akan di beritahu via PM.")
		elif args[0] in ("no", "off", "ga"):
			sql.set_feds_setting(user.id, False)
			send_message(update.effective_message, "Pelaporan federasi mati! Setiap ada pengguna yang di fban/unfban anda tidak akan di beritahu via PM.")
		else:
			send_message(update.effective_message, "Tolong masukan `ya`/`on`/`ga`/`off`"), parse_mode="markdown"
	else:
		getreport = sql.user_feds_report(user.id)
		send_message(update.effective_message, "Preferensi laporan federasi anda saat ini: `{}`").format(getreport), parse_mode="markdown"
...
def fed_chats(update, context):
	...
	if len(getlist) == 0:
		send_message(update.effective_message, "Tidak ada obrolan yang bergabung pada federasi {}").format(info['fname']), parse_mode=ParseMode.HTML
		return

	text = (update.effective_message, "<b>Obrolan yang bergabung pada federasi {}:</b>\n").format(info['fname'])
	...
	try:
		...
		with BytesIO(str.encode(cleantext)) as output:
			...
			update.effective_message.reply_document(document=output, filename="fedchats.txt",
													caption=(update.effective_message, "Berikut adalah daftar obrolan yang bergabung federasi {}.").format(info['fname']))
...
def fed_import_bans(update, context, chat_data):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		...
		try:
			...
			send_message(update.effective_message, "Coba unduh dan unggah ulang filenya, yang ini sepertinya rusak!")
			return
		...
		if fileformat == 'json':
			...
			text = (update.effective_message, "Berkas blokir berhasil diimpor. {} orang diblokir.").format(success)
			if failed >= 1:
				text += (update.effective_message, " {} gagal di impor.").format(failed)
			get_fedlog = sql.get_fed_log(fed_id)
			if get_fedlog:
				if eval(get_fedlog):
					teks = (update.effective_message, "Federasi *{}* telah berhasil mengimpor data. {} di blokir").format(getfed['fname'], success)
					if failed >= 1:
						teks += (update.effective_message, " {} gagal di impor.").format(failed)
...
def del_fed_button(update, context):
	...
	if fed_id == 'cancel':
		query.message.edit_text(update.effective_message, "Penghapusan federasi dibatalkan")
		return
	...
	if getfed:
		...
		if delete:
			query.message.edit_text(update.effective_message, "Anda telah menghapus federasi Anda! Sekarang semua Grup yang terhubung dengan `{}` sekarang tidak memiliki federasi.").format(getfed['fname']), parse_mode='markdown'
	...
	if user_id and user_id != "error":
		if len(args) == 2 and args[0].isdigit():
			...
			if fbantime:
				...
				fbantime = "Unavaiable"
			if user_name == False:
				send_message(update.effective_message, "Federasi {} tidak di temukan!").format(fed_id), parse_mode="markdown"
				return
			if user_name == "" or user_name == None:
				user_name = (update.effective_message, "Dia")
			if not reason:
				send_message(update.effective_message, "{} belum di larang di federasi ini!").format(user_name)
			else:
				teks = (update.effective_message, "{} di larang di federasi ini karena:\n`{}`\n*Di banned pada:* `{}`").format(user_name, reason, fbantime)
		...
		if len(fbanlist) == 0:
			send_message(update.effective_message, "{} belum di larang di federasi mana pun!").format(user_name)
			return
		else:
			teks = (update.effective_message, "{} sudah di larang di federasi ini:\n").format(user_name)
			for x in fbanlist:
				teks += "- `{}`: {}\n".format(x[0], x[1][:20])
			teks += (update.effective_message, "\nJika anda ingin mengetahui lebih lanjut tentang alasan fedban dengan spesifik, gunakan /fbanstat <FedID>")
			send_message(update.effective_message, teks, parse_mode="markdown")
		...
		if not reason:
			send_message(update.effective_message, "{} tidak di larang di federasi ini").format(name)
			return
		send_message(update.effective_message, "{} di larang di federasi ini karena:\n`{}`\n*Di banned pada:* `{}`").format(name, reason, fbantime), parse_mode="markdown"
...
def set_fed_log(update, context):
	...
	if args:
		...
		if not isowner:
			send_message(update.effective_message, "Hanya pencipta federasi yang bisa menetapkan log federasi.")
			return
		setlog = sql.set_fed_log(args[0], chat.id)
		if setlog:
			send_message(update.effective_message, "Log federasi `{}` telah di setel pada {}").format(fedinfo['fname'], chat.title), parse_mode="markdown"
	else:
		send_message(update.effective_message, "Anda belum memberikan ID federasinya!")
...
def unset_fed_log(update, context):
	...
	if args:
		...
		if setlog:
			send_message(update.effective_message, "Log federasi `{}` telah di cabut pada {}").format(fedinfo['fname'], chat.title), parse_mode="markdown"
	else:
		send_message(update.effective_message, "Anda belum memberikan ID federasinya!")
...
def subs_feds(update, context):
	...
	if args:
		...
		if subfed:
			send_message(update.effective_message, "Federasi `{}` telah mengikuti federasi `{}`. Setiap ada fedban dari federasi tersebut, federasi ini juga akan banned pengguna tsb.").format(fedinfo['fname'], getfed['fname']), parse_mode="markdown"
			get_fedlog = sql.get_fed_log(args[0])
			if get_fedlog:
				if int(get_fedlog) != int(chat.id):
					context.bot.send_message(get_fedlog, (update.effective_message, "Federasi `{}` telah mengikuti federasi `{}`").format(fedinfo['fname'], getfed['fname']), parse_mode="markdown")
		else:
			send_message(update.effective_message, "Federasi `{}` sudah mengikuti federasi `{}`.").format(fedinfo['fname'], getfed['fname']), parse_mode="markdown"
	else:
		send_message(update.effective_message, "Anda belum memberikan ID federasinya!")
...
def unsubs_feds(update, context):
	...
	if args:
		...
		if subfed:
			send_message(update.effective_message, "Federasi `{}` sudah tidak mengikuti `{}` lagi.").format(fedinfo['fname'], getfed['fname']), parse_mode="markdown"
			get_fedlog = sql.get_fed_log(args[0])
			if get_fedlog:
				if int(get_fedlog) != int(chat.id):
					context.bot.send_message(get_fedlog, (update.effective_message, "Federasi `{}` sudah tidak mengikuti `{}`").format(fedinfo['fname'], getfed['fname']), parse_mode="markdown")
		else:
			send_message(update.effective_message, "Federasi `{}` tidak mengikuti federasi `{}`.").format(fedinfo['fname'], getfed['fname']), parse_mode="markdown"
	else:
		send_message(update.effective_message, "Anda belum memberikan ID federasinya!")
...
def get_myfedsubs(update, context):
	...
	if len(getmy) == 0:
		send_message(update.effective_message, "Federasi `{}` tidak mengikuti federasi apapun.").format(fedinfo['fname']), parse_mode="markdown"
		return
	else:
		listfed = (update.effective_message, "Federasi `{}` mengikuti federasi berikut ini:\n").format(fedinfo['fname'])
		for x in getmy:
			listfed += "- `{}`\n".format(x)
		listfed += (update.effective_message, "\nUntuk info federasi, ketik `/fedinfo <fedid>`. Untuk berhenti berlangganan ketik `/unsubfed <fedid>`.")
...
def __stats__():
	...
	return (OWNER_ID, "{} pengguna di fbanned, pada {} federasi").format(len(all_fbanned), len(all_feds))
...
def __user_info__(user_id, chat_id):
	...
	if fed_id:
		...
		if int(info['owner']) == user_id:
			text = (chat_id, "Pengguna ini adalah owner di federasi saat ini: <b>{}</b>.").format(infoname)
		elif is_user_fed_admin(fed_id, user_id):
			text = (chat_id, "Pengguna ini adalah admin di federasi saat ini: <b>{}</b>.").format(infoname)

		elif fban:
			text = (chat_id, "Dilarang di federasi saat ini: <b>Ya</b>")
			text += (chat_id, "\n<b>Alasan:</b> {}").format(fbanreason)
		else:
			text = (chat_id, "Dilarang di federasi saat ini: <b>Tidak</b>")
	else:
		text = ""
	return text
