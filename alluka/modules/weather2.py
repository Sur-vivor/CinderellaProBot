...
def cuaca(update, context):
    ...
    if location.lower() == context.bot.first_name.lower():
        send_message(update.effective_message, "Saya akan terus mengawasi di saat senang maupun sedih!")
    ...
    try:
        ...
        send_message(update.effective_message, "{} hari ini sedang {}, sekitar {}°C.\n").format(lokasinya,
                statusnya, temperatur)

    except pyowm.exceptions.api_call_error.APICallError:
        send_message(update.effective_message, "Tulis lokasi untuk mengecek cuacanya")
    except pyowm.exceptions.api_response_error.NotFoundError:
        send_message(update.effective_message, "Maaf, lokasi tidak ditemukan 😞")
    else:
        return
...
def accuweather(update, context):
    ...
    args = context.args
    if not args:
        return send_message(update.effective_message, "Masukan nama lokasinya untuk mengecek cuacanya!")
    location = " ".join(args)
    if location.lower() == context.bot.first_name.lower():
        send_message(update.effective_message, "Saya akan terus mengawasi di saat senang maupun sedih!")
        context.bot.send_sticker(update.effective_chat.id, BAN_STICKER)
    ...
    if True:
        url = "http://api.accuweather.com/locations/v1/cities/search.json?q={}&apikey={}".format(location, API_WEATHER)
        headers = {'Content-type': 'application/json'}
        r = requests.get(url, headers=headers)
        try:
            data = r.json()[0]
        except:
            return send_message(update.effective_message, "Maaf, lokasi tidak ditemukan 😞")
        locid = data.get('Key')
        weatherlang = (update.effective_message, "weather_lang")
        urls = "http://api.accuweather.com/currentconditions/v1/{}.json?apikey={}&language={}&details=true&getphotos=true".format(locid, API_WEATHER, weatherlang)
